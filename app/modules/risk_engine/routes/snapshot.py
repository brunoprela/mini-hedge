"""FastAPI routes for risk snapshots, VaR, stress tests, and factor decomposition."""
...
from app.modules.risk_engine.dependencies import get_risk_snapshot_service
...
# ---------------------------------------------------------------------------
# Routes — Snapshot / VaR / Stress / Factors
# ---------------------------------------------------------------------------
...
async def get_risk_snapshot(
    ...
    return await risk_snapshot_service.get_latest_snapshot(portfolio_id, session=session)
...
async def get_risk_history(
    ...
    return await risk_snapshot_service.get_snapshot_history(
...
async def calculate_var(
    ...
    return await risk_snapshot_service.calculate_var(
        portfolio_id,
        method=body.method,
...
async def run_predefined_stress_tests(
    ...
    for scenario in PREDEFINED_SCENARIOS:
        result = await risk_snapshot_service.run_stress_test(
            portfolio_id, scenario, session=session
        )
...
async def run_custom_stress_test(
    ...
    return await risk_snapshot_service.run_stress_test(portfolio_id, scenario, session=session)
...
async def get_factor_decomposition(
    ...
    return await risk_snapshot_service.calculate_factor_model(portfolio_id, session=session)
