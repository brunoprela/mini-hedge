...
class AttributionService:
    ...
    def __init__(
        self,
        *,
        bf_repo: BrinsonFachlerRepository,
        bf_sector_repo: BrinsonFachlerSectorRepository,
        rb_repo: RiskBasedRepository,
        rfc_repo: RiskFactorContributionRepository,
        cum_repo: CumulativeAttributionRepository,
        ...
        self._rb_repo = rb_repo
        self._rfc_repo = rfc_repo
        self._cum_repo = cum_repo
    ...
    async def calculate_cumulative(
        ...
        *,
        ...
        records = await self._bf_repo.get_by_portfolio(
            portfolio_id, period_start, period_end, session=session
        )

        period_results: list[BrinsonFachlerResult] = []
        for r in records:
            sectors_records = await self._bf_sector_repo.get_by_bf_result(r.id, session=session)
            sectors = [
                SectorAttribution(
                    sector=s.sector,
