...
class QuantResearchService:
    ...
    def __init__(
        ...
        factor_exp_repo: FactorExposureRepository,
        factor_ret_repo: FactorReturnRepository,
        ...
        self._factor_def_repo = factor_def_repo
        self._factor_exp_repo = factor_exp_repo
        self._factor_ret_repo = factor_ret_repo
    ...
    async def list_factors(self, *, session: AsyncSession | None = None) -> list[FactorDefinition]:
        records = await self._factor_def_repo.list_all(session=session)
        return [
            FactorDefinition(
                id=UUID(r.id),
                name=r.name,
    ...
    async def analyze_factor(
        ...
        *,
        ...
        returns = await self._factor_ret_repo.get_by_factor(
            factor_record.id,
        ...
        all_factors = await self._factor_def_repo.list_all(session=session)
        factor_return_series: dict[str, list[Decimal]] = {}
        for f in all_factors:
            f_returns = await self._factor_ret_repo.get_by_factor(
        ...
        today = date.today()
        exp_records = await self._factor_exp_repo.get_by_factor_date(
    ...
    async def decompose_portfolio(
        ...
        *,
        ...
        for fname in factor_names:
            factor_record = await self._factor_def_repo.get_by_name(fname, session=session)
            if factor_record is None:
                continue
            today = date.today()
            exp_records = await self._factor_exp_repo.get_by_factor_date(
