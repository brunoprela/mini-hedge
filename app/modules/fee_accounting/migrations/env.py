"""Alembic environment for fee accounting tables — per-fund schema isolation.

Same pattern as orders: tables live in fund-specific schemas
(fund_alpha, fund_beta, etc.) via schema_translate_map.
"""
...
from app.modules.fee_accounting.models import Base
...
def run_migrations_offline() -> None:
    ...
    context.configure(
        ...
        version_table="alembic_version_fee_accounting",
...
def run_migrations_online() -> None:
    ...
    if schema == DEFAULT_SCHEMA:
        raise RuntimeError(
            "Fee accounting migrations must target a per-fund schema. "
