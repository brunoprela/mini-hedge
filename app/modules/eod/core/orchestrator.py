...
if TYPE_CHECKING:
    ...
    from app.modules.investor_operations.services import RedemptionService, SubscriptionService
    ...
    async def _execute_step(
        ...
        *,
        ...
        if step == EODStepName.CAPITAL_ALLOCATION:
            if self._capital_transaction_service is None:
                return {"message": "capital_transaction_service_not_configured"}
        ...
        if step == EODStepName.DEALING_DATE_EXECUTION:
            if self._subscription_service is None and self._redemption_service is None:
                return {"message": "investor_ops_services_not_configured"}
            ...
            if self._subscription_service is not None:
                subs = await self._subscription_service.execute_subscriptions(
                    dealing_date=business_date,
                    nav_per_share=nav_per_share,
                    portfolio_id=portfolio_id,
                )
            reds = []
            if self._redemption_service is not None:
                reds = await self._redemption_service.execute_redemptions(
                    dealing_date=business_date,
                    nav_per_share=nav_per_share,
                    portfolio_id=portfolio_id,
                )
