...
class FeatureStoreService:
    ...
    async def deprecate_feature(
        ...
        *,
        ...
        record = await self._repo.get_by_name(feature_name, session=session)
        if record is None:
    ...
    async def compute_feature(
        ...
        *,
        ...
        await self._repo.save_values([val_record], session=session)
    ...
    async def compute_features_batch(
        ...
        *,
        ...
        for name in feature_names:
            rec = await self._repo.get_by_name(name, session=session)
            if rec is None:
        ...
        if val_records:
            await self._repo.save_values(val_records, session=session)
    ...
    async def get_feature_vector(
        ...
        *,
        ...
        raw = await self._repo.get_feature_vector(entity_id, feature_ids, session=session)
    ...
    async def get_feature_stats(
        ...
        *,
        ...
        rec = await self._repo.get_by_name(feature_name, session=session)
        if rec is None:
            msg = f"Feature '{feature_name}' not found"
            raise ValueError(msg)

        stats = await self._repo.get_stats(rec.id, session=session)
        return FeatureStats(
            feature_name=feature_name,
            count=stats["count"],
    ...
    async def create_feature_set(
        ...
        *,
        ...
        await self._repo.create_feature_set(record, session=session)
    ...
    async def list_feature_sets(
        ...
        *,
        ...
        records = await self._repo.list_feature_sets(session=session)
