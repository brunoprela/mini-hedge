...
@pytest.fixture
def feed_repo() -> AsyncMock:
    return AsyncMock()


@pytest.fixture
def point_repo() -> AsyncMock:
    return AsyncMock()
...
class TestIngestData:
    async def test_returns_count_of_ingested_points(
        self, service: AltDataService, feed_repo: AsyncMock, point_repo: AsyncMock
    ):
        feed_id = uuid4()
        feed_repo.get_feed.return_value = _make_feed_record(id=str(feed_id), record_count=0)
        ...
        assert count == 2
        point_repo.insert_data_points.assert_called_once()
    ...
    async def test_publishes_audit_event_on_ingest(
        ...
        feed_id = uuid4()
        feed_repo.get_feed.return_value = _make_feed_record(id=str(feed_id), record_count=5)
    ...
    async def test_updates_feed_record_count(
        self, service: AltDataService, feed_repo: AsyncMock, point_repo: AsyncMock
    ):
        feed_id = uuid4()
        feed_repo.get_feed.return_value = _make_feed_record(id=str(feed_id), record_count=10)
        ...
        feed_repo.update_feed.assert_called_once()
        _, call_kwargs = feed_repo.update_feed.call_args
...
class TestQueryData:
    async def test_returns_data_points(
        self, service: AltDataService, feed_repo: AsyncMock, point_repo: AsyncMock
    ):
        feed_id = uuid4()
        point_repo.get_data_points.return_value = [
    ...
    async def test_metadata_mapped_from_extra_metadata(
        ...
        point_repo.get_data_points.return_value = [pt]
...
class TestCollectSentiment:
    ...
    async def test_no_providers_returns_empty(
        self, feed_repo: AsyncMock, point_repo: AsyncMock, event_bus: InProcessEventBus
    ):
        svc = AltDataService(
            feed_repo=feed_repo, point_repo=point_repo, providers=[], session_factory=MagicMock()
        )
