...
from app.modules.ai_analysis.interface import AnalysisRequest, AnalysisType, SentimentScore
...
@pytest.fixture
def note_repo_side_effect(analysis_repo: AsyncMock) -> AsyncMock:
    async def _save_note(record, *, session=None):
        record.id = FAKE_UUID
        record.created_at = NOW

    analysis_repo.save_note.side_effect = _save_note
...
class TestCreateResearchNote:
    async def test_note_persisted_and_returned(
        self, service: AIAnalysisService, analysis_repo: AsyncMock
    ):
        async def _save_note(record, *, session=None):
            record.id = FAKE_UUID
            record.created_at = NOW
        ...
        analysis_repo.save_note.assert_awaited_once()
...
class TestEventPublishing:
    ...
    async def test_research_note_created_event_published(
        self, service: AIAnalysisService, analysis_repo: AsyncMock, capture: EventCapture
    ):
        async def _save_note(record, *, session=None):
            record.id = FAKE_UUID
            record.created_at = NOW
