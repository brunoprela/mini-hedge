...
from app.modules.feature_store.models import FeatureDefinitionRecord
...
@pytest.fixture
def service(
    repo: AsyncMock, engine: FeatureComputeEngine, event_bus: InProcessEventBus
) -> FeatureStoreService:
...
class TestRegisterFeature:
    async def test_returns_feature_definition(self, service: FeatureStoreService, repo: AsyncMock):
        repo.create_definition.side_effect = lambda r, **kw: _seed_def_record(r)
    ...
    async def test_publishes_audit_event(
        self, service: FeatureStoreService, repo: AsyncMock, capture: EventCapture
    ):
        repo.create_definition.side_effect = lambda r, **kw: _seed_def_record(r)
...
class TestListFeatures:
    async def test_returns_all_features(self, service: FeatureStoreService, repo: AsyncMock):
        repo.list_definitions.return_value = [
    ...
    async def test_filters_by_entity_type(self, service: FeatureStoreService, repo: AsyncMock):
        repo.list_definitions.return_value = [_make_def_record("vol_30")]

        features = await service.list_features(entity_type="instrument")

        repo.list_definitions.assert_called_once_with(
...
class TestComputeFeature:
    async def test_computes_sma_and_stores_value(
        self, service: FeatureStoreService, repo: AsyncMock
    ):
        record = _make_def_record("sma_5", expression="sma(prices, 3)")
        repo.get_by_name.return_value = record
        ...
        repo.save_values.assert_called_once()

    async def test_unknown_feature_raises(self, service: FeatureStoreService, repo: AsyncMock):
...
class TestComputeFeaturesBatch:
    async def test_publishes_audit_event_after_batch(
        ...
        repo.get_by_name.return_value = rec
